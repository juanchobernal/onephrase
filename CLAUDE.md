...
6. [x] **Cargar el `.ehpk` (flujo "Private testing") — HECHO (2026-06-25): corre bien instalada desde el paquete.** Subir el `.ehpk` en `hub.evenrealities.com/login` → proyecto → pestaña **Private builds**. Luego en la app: **Me → Apps → Private builds → Install** (requiere Developer Mode activo, ver gotcha). NO tiene hot-reload (~10s por ciclo).

   **⚠️ GOTCHA AL ACTUALIZAR EL `.ehpk` (2026-06-25):** la app del teléfono identifica el paquete por **`app.json` `version` (semver)**, NO por la etiqueta `BUILD` del header (esa es solo del JS para dev/QR). Si re-empacas con la **misma** `version`, la app cree que ya la tiene y **no actualiza** (sigue mostrando el bundle viejo — le pasó al usuario: subió "0.1.0" encima de "0.1.0" y seguía viendo v17). **Para publicar un update: SUBIR `app.json` `version`** (p. ej. 0.1.0→0.2.0), luego `npm run build` → `npx evenhub pack app.json dist -o onephrase-<version>.ehpk` → re-subir a Private builds → reinstalar (desinstalar la vieja primero si no ofrece update). El `BUILD` del header es independiente (cache-bust del QR en dev).
