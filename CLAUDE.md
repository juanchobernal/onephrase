...
**Síntoma:** en las **gafas físicas** (no en el simulador), los modos de palabra-por-palabra (1 translate-word, 2 transcribe-word) muestran **solo UNA palabra** de la frase y a veces se **congelan en una palabra vieja** mientras se sigue hablando. El **simulador del Mac muestra los 3 modos perfecto** — son instancias independientes (simulador = render instantáneo; gafas = escritura por **Bluetooth LE**, lento).

**Hipótesis principal (BLE congestión + deadlock de `inflight`):** en `src/glasses-render.ts`, `GlassesStage.flush()` usa un candado `inflight` y hace `await bridge.textContainerUpgrade(...)`. Si una escritura BLE **nunca resuelve** (cola congestionada), `inflight` se queda `true` para siempre → render congelado. Los modos 1/2 escriben muy seguido (modo 1 cada `WORD_DRIP_MS=350`ms; modo 2 en cada palabra interina) → saturan el BLE. El **modo 3 (translate-sentence) escribe 1 vez por frase** → debería ir estable en hardware (PENDIENTE confirmarlo con el usuario: ciclar con single-tap a "TRADUCCIÓN · FRASE" y validar).

**Fix propuesto (1 archivo, ~8 líneas, NO aplicado aún):** poner timeout (~2s) a la escritura vía `Promise.race`, para que `inflight` siempre se libere aunque la escritura BLE se cuelgue. Y si el modo 3 confirma que es congestión, **bajar el ritmo del drip en hardware** (subir `WORD_DRIP_MS` o coalescer más). NO se editó esperando la prueba del modo 3 del usuario.

**Cómo retomar:** dev server probablemente caído — relanzar `npm run dev`, regenerar QR (`npx evenhub qr --url http://<IP>:5173` desde la carpeta), reconectar gafas, y pedir al usuario que confirme modo 3 antes de aplicar el fix. HMR recarga las gafas solo al guardar.
